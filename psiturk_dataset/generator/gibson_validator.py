...
from habitat.datasets.object_nav.create.object_nav_generator import is_compatible_episode

config = habitat.get_config("configs/tasks/objectnav_gibson_il.yaml")
...
def generate_trajectories(cfg, episode_path, output_prefix="s_path", enable_turns=False, output_path="valid_episodes.json"):
    possible_actions = cfg.TASK.POSSIBLE_ACTIONS
    with habitat.Env(cfg) as env:
        goal_radius = 0.1
        spl = 0
        total_compatible = 0.0
        total_episodes = 0.0
        fixed_episodes = 0
        turn_fix_episodes = 0
        ...
        for ep_id in range(len(env.episodes)):
            ...
            if sim.island_radius(episode.start_position) < ISLAND_RADIUS_LIMIT:
                continue
            ...
            total_compatible += is_compatible
            ...
            if ep_id % 100 == 0:
                print("Success: {}, {}, {}".format(total_compatible/total_episodes, total_compatible, total_episodes))
                write_json(success_episodes, output_path)

        
        print("Total episodes: {}".format(total_episodes))

        print("\n\nEpisode success: {}".format(total_compatible / total_episodes))
        print("Total episode success: {}".format(total_compatible))
        print("Success: {}, {}, {}".format(total_compatible/total_episodes, total_compatible, total_episodes))
        write_json(success_episodes, output_path)
...
def main():
    ...
    parser.add_argument(
        "--output-path", type=str, default="valid_episodes.json"
    )
