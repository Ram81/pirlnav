...
COMPRESSION = '.gz'
OUTPUT_OBJ_FOLDER = "./data/datasets/objectnav/mp3d//v1_allobjs_cats_90_aabb_round_obb_fixed"
OUTPUT_JSON_FOLDER = "./data/datasets/objectnav/mp3d/v1_floatencoded_cats_90_normalized_round_obb_fixed"
...
def get_objnav_config(i, scene):
    ...
    CFG_EQA = "configs/tasks/obj_nav_mp3d.yaml"
...
def get_gravity_mobb(object_obb : habitat_sim.geo.OBB):
    bounding_area = [(object_obb.local_to_world @ np.array([x, y, z, 1]))[:-1] for x,y,z in 
        itertools.product(*([[-1, 1]] * 3))]
    bounding_area = np.array(bounding_area, dtype=np.float32)
    # print('Bounding Area: %s' % bounding_area)
    #TODO Maybe Cache this
    return habitat_sim.geo.compute_gravity_aligned_MOBB(habitat_sim.geo.GRAVITY, bounding_area)
...
def generate_scene(args):
    ...
    for source_id, source_obj in enumerate(
        tqdm.tqdm(sim.semantic_annotations().objects[1:], desc='Generating object data')
    ):
    ...
    if True:
        total_objs_json = 1000 if split != 'val' else 200
        ...
        with tqdm.tqdm(
            total=total_objs_json, desc=scene
        ) as pbar:
            ...
            for goals in goals_by_class.values():
                ...
                eps_generated = 0
                if True:
                    eps_per_obj= int(len(goals) / total_valid_objs * total_objs_json + 0.5)
                else:
                    eps_per_obj = 500
                try:
...
if __name__ == "__main__":
    ...
    for split in splits:
        ...
        dset.category_to_mp3d_category_id = (
            category_to_mp3d_category_id
        )
        ...
        if split != "train":
            ...
            with mp_ctx.Pool(CPU_THREADS) as pool, tqdm.tqdm(
                ...
                for i, dset2 in enumerate(
                    pool.map(read_dset, jsons_gz)
                ):
                    if i == 0 and split == "val":
                        MINI_SAMPLE = 30
                        print('Sampling %d episodes for minival' % MINI_SAMPLE)
                        min_dset_fname = f"{OUTPUT_JSON_FOLDER}/mini_{split}/mini_{split}.json{COMPRESSION}"
                        ...
                        min_dset.episodes = list(dset2.get_episode_iterator(num_episode_sample=MINI_SAMPLE, cycle=False))
